...
class Encoder(nn.Module):
    def __init__(self, z_size=64):
        ...
        self.shared_layers = nn.Sequential(
            EncoderBlock(channel_in=128, channel_out=128),
            EncoderBlock(channel_in=128, channel_out=256),
            EncoderBlock(channel_in=256, channel_out=256)
        )

        self.fc = nn.Sequential(
            nn.Linear(in_features=1 * 64 * 64, out_features=512, bias=False),
    ...
    def forward(self, anatomical_img, fat_fraction_img):
        ...
        combined_encoding = torch.cat((anatomical_encoded, fat_fraction_encoded), dim=1)
        
        combined_encoding = self.shared_layers(combined_encoding).detach()  # Added detach
        combined_encoding = combined_encoding.view(len(combined_encoding), -1).detach()
...
class VaeGan(nn.Module):
    def __init__(self, z_size=64, recon_level=3):
    ...
    def forward(self, anatomical_img, fat_fraction_img=None, gen_size=10):
        if self.training:
            ...
            ten_sampled = torch.randn(len(anatomical_img), self.z_size, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'), requires_grad=True)
    ...
    def loss(data_target, out, out_layer_original, out_layer_predicted, out_layer_sampled, 
        ...
        return nle_value, kl_value, mse_value_1, mse_value_2, bce_dis_original_value, bce_dis_sampled_value, \
